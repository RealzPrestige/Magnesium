...
import dev.zprestige.magnesium.settings.impl.SliderFloat
...
class SliderFloatButton(private val slider: SliderFloat, x: Float, y: Float, width: Float, height: Float) : SettingButton(
    slider,
    x,
    y,
    width,
    ...
    private fun updateValue(mouseX: Int) {
        ...
        val min: Float = slider.min
        val max: Float = slider.max
        val value = roundNumber((diff / w * (max - min) + min).toDouble())
        slider.value = if (diff == 0f) {
            slider.min
        } else {
            value.coerceAtMost(slider.max)
        }
    }
